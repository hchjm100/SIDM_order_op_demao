...
double MOverR3(FLOAT r) 
{
   // Hernquist median 
   double rhoh= 0.000228893;
   double ra=12.47;
   double valHern = 2*3.14159265358979*rhoh*pow(ra,3)/(r*pow(r+ra,2));

   //double h = All.ForceSoftening[1]; //SofteningHalo*2.8
   //double h_inv,h3_inv,u;
   //h_inv = 1.0 / h;
   //h3_inv = h_inv * h_inv * h_inv;
   //double valHalo=(4*3.14159265358979*rhos*pow(rs,3)*(-(r/(r + rs)) - log(rs/(r + rs)) ));

   ////printf("XXXXX: r %g, rho %g, rh %g, M %g \n",r/0.7,rhoh*0.7*0.7,rh/0.7,valHern/0.7);
...
void gravity_tree(void)
{
  ...
  for(i=0; i < NumPart; i++)
     if(P[i].Ti_endstep==All.Ti_Current)
     {
        double rad   = sqrt((P[i].Pos[0]) * (P[i].Pos[0]) + (P[i].Pos[1]) * (P[i].Pos[1])
                  + (P[i].Pos[2]) * (P[i].Pos[2]));
        if(rad!=0){
           double f=1;
           //if(All.Time<4) f=All.Time/4;
           for(j=0; j<3; j++){
              P[i].GravAccel[j] -= f*MOverR3(rad) * (P[i].Pos[j]) * All.G;
