...
void init(void)
{
  ...
  // Effective only for cosmological simulation!!!! no worry...
    {
      for(i = 0; i < NumPart; i++){
        // DY
        P[i].Mass*=All.HubbleParam; 
	for(j = 0; j < 3; j++){
          // DY
          P[i].Pos[j] = P[i].Pos[j]*All.HubbleParam/All.Time;
          // Gadget2 velocity variable is actually the canonical momentum p=a^2 dot{x}
          // The IC file and snapshot file is expected to use w = sqrt(a) dot{x}
	  // P[i].Vel[j] *= sqrt(All.Time) * All.Time; 
	  // -------------------------------------------------------------------------
	  // We use SphereIC to generate the initial condition, where the velocity is expected to be peculiar velocity = a dot{x}
          P[i].Vel[j] *= All.Time;
        }
      }
    }
  ...
  //N_DM = NumPart - N_gas; // DY
  //sidmngb_treebuild();		/* will build tree */
